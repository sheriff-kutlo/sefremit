"""
Django settings for sefremit project.
...
WSGI_APPLICATION = 'sefremit.wsgi.application'
...
LOGGING = {
    ...
    'formatters': {
        'simple': {
            'format': '%(asctime)s %(levelname)s %(message)s',  # Specify the desired format
            'datefmt': '%Y-%m-%d %H:%M:%S',  # Optional: Date/time format for asctime
        },
    },
    'handlers': {
        'file': {
            ...
            # 'filename': '/var/log/gunicorn/sefalana_trading.log',
            'formatter': 'simple',
        },
        'console': {
            'level': 'DEBUG',  # Set the desired logging level for the console
            'class': 'logging.StreamHandler',
            'formatter': 'simple',
        },
    },
    'loggers': {
        'django': {  # Disable the django logger
            'handlers': [],
            'propagate': False,
        },
        'django.request': {  # Disable request logging if not needed
            'handlers': [],
            'propagate': False,
        },
        'chatbot_catalog': {  # Configure your app's logger
