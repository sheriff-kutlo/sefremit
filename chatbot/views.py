            ...
            try:
                ...
                if 'interactive' in json_data['entry'][0]['changes'][0]['value']['messages'][0]:
                    ...
                    if interactive_data['type'] == 'list_reply':
                        ...
                        if flow_token == "file_complaint_token":
                           send_message("Your Complaint has been successfully submited", message_from)
                           
                        elif flow_token == "file_complaint_token": 
                            send_message("We have received your complaint. Thank you", message_from)
                            

                        elif flow_token == "bw_registration_token":
                            send_message("Domain airbotswana.co.bw registered with MarkMonitor Inc., created on 2003-06-12, expires on 2026-06-11. Status: clientDeleteProhibited, clientTransferProhibited, clientUpdateProhibited.", message_from)
                ...
                if 'text' in json_data['entry'][0]['changes'][0]['value']['messages'][0]:
                    message = json_data['entry'][0]['changes'][0]['value']['messages'][0]
                    metadata = json_data['entry'][0]['changes'][0]['value']['metadata']
                    contact = json_data['entry'][0]['changes'][0]['value']['contacts'][0]
                    ...
                    logger.info(f"[Text] Message Body: {body}")
                    logger.info(f"[Text] Message ID: {message_id}")
                    logger.info(f"[Text] From: {message_from}")
                    logger.info(f"[Text] Customer Name: {name}")
                    logger.info(f"[Text] Display Phone Number: {display_phone_number}")
